...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    if (materiaIds.length === 0) {
      return createSuccessResponse({ temas: [], total: 0, sin_guia: 0, con_guia: 0, con_sesiones: 0 });
    }
    ...
    const temasConEstatus = (temas || []).map((tema: any) => {
      ...
      return {
        ...
        grupo: (() => {
          const grupo = asignacion?.grupos ? (Array.isArray(asignacion.grupos) ? asignacion.grupos[0] : asignacion.grupos) : null;
          return grupo ? {
            id: grupo.id,
            nombre: grupo.nombre,
            grado: grupo.grado,
